...
#define FILENAME "D:\\code\\StudentManegement\\studentlist.txt" // 缺省数据文件名
...
void Select_ManageMenu(struct student stu[], char szCourseName[][STRLENGTH], int *npStudentNum)
{
    ...
    while (1)
    {
        ...
        case 3:
            ...
            break;
        case 0:
            Select_MainMenu(stu, szCourseName, npStudentNum); // 返回一级主菜单函数
            break;
...
void Select_SortMenu(struct student stu[], char szCourseName[][STRLENGTH], int nStudentNum)
{
    ...
    while (1)
    {
        ...
        case 4: // 添加完函数后这里就可以解开注释了
            ...
            break;
        case 0:
            Select_MainMenu(stu, szCourseName, &nStudentNum); // 返回一级主菜单选项显示
            break;
        default:
            printf("\t\t选择有误，请重选！\n");
        } // switch结束
...
void Select_QueryMenu(struct student stu[], char szCourseName[][STRLENGTH], int nStudentNum)
{
    ...
    while (1)
    {
        ...
        {
        case 1:
            FindByNumber(stu, szCourseName, nStudentNum); // 按学号查询学生信息
            system("PAUSE");
            break;
        case 2:
            FindByName(stu, szCourseName, nStudentNum); // 按姓名查询学生信息
            system("PAUSE");
            break;
        case 0:
            Select_MainMenu(stu, szCourseName, &nStudentNum); // 返回一级主菜单选项
            break;
        default:
            printf("\t\t选择有误，请重选！\n");
        } // switch结束
